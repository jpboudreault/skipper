	...
	async function saveAll() {
		...
		try {
			...
			if (res.ok)
				...
				saving = false;
				alert(translate('pitching_failed_save'));
			}
		} catch (e) {
			saving = false;
			console.error(e);
...
<div class="card bg-base-100 border-base-300 overflow-hidden border shadow-xl">
	<div class="border-base-200 bg-base-200/40 flex items-center justify-between border-b px-6 py-4">
		<div>
			<h2 class="text-base-content text-xl font-bold">{$t('pitching_title')}</h2>
			<p class="text-base-content/70 mt-1 text-sm">{$t('pitching_description')}</p>
		</div>
		<div class="flex gap-2">
			<button onclick={() => addRow()} class="btn btn-neutral btn-sm"
				>{$t('pitching_add_row')}</button
			>
	...
	{#if loading}
		<div class="p-16 text-center">
			...
			<p class="text-base-content/60 mt-2 text-sm">{$t('pitching_loading')}</p>
		</div>
	{:else}
		<div class="overflow-x-auto">
			<table class="table w-full">
				<thead class="bg-base-200">
					<tr>
						<th class="text-base-content w-40 font-bold">{$t('pitching_pitcher')}</th>
						<th class="text-base-content w-16 text-center font-bold"
							>{$t('pitching_from_inning')}</th
						>
						<th class="text-base-content w-16 text-center font-bold">{$t('pitching_to_inning')}</th>
						<th class="text-base-content w-16 text-center font-bold">{$t('pitching_ip_outs')}</th>
						<th class="text-base-content w-14 text-center font-bold">R</th>
						<th class="text-base-content w-14 text-center font-bold">K</th>
						<th class="text-base-content w-14 text-center font-bold">BB</th>
						<th class="text-base-content w-14 text-center font-bold">HBP</th>
						<th class="text-base-content w-16 text-center font-bold">{$t('pitching_pitches')}</th>
						<th class="text-base-content w-12 text-center font-bold"></th>
					</tr>
				</thead>
				<tbody>
						...
						<tr class="hover:bg-base-200/50 transition-colors">
							...
							<td class="text-center"
								><input
									type="number"
									min="0"
									value={app.ip_outs === 0 ? '' : app.ip_outs}
									oninput={(e) => {
										const v = (e.currentTarget as HTMLInputElement).value;
										app.ip_outs = parseInt(v) || 0;
										isDirty = true;
									}}
									class="input input-bordered input-sm w-16 text-center"
								/></td
							...
							<td class="text-center"
								><input
									type="number"
									min="0"
									value={app.runs_allowed === 0 ? '' : app.runs_allowed}
									oninput={(e) => {
										const v = (e.currentTarget as HTMLInputElement).value;
										app.runs_allowed = parseInt(v) || 0;
										isDirty = true;
									}}
							...
							<td class="text-center"
								><input
									type="number"
									min="0"
									value={app.hbp === 0 ? '' : app.hbp}
									oninput={(e) => {
										const v = (e.currentTarget as HTMLInputElement).value;
										app.hbp = parseInt(v) || 0;
										isDirty = true;
									}}
									class="input input-bordered input-sm w-14 text-center"
								/></td
							...
							<td class="text-center"
								><input
									type="number"
									min="0"
									value={app.pitch_count === 0 ? '' : app.pitch_count}
									placeholder="—"
									oninput={(e) => {
										const v = (e.currentTarget as HTMLInputElement).value;
										app.pitch_count = parseInt(v) || 0;
										isDirty = true;
									}}
									class="input input-bordered input-sm w-16 text-center"
								/></td
							>
							<td class="text-center">
								<button onclick={() => removeRow(i)} class="btn btn-ghost btn-error btn-xs"
									>✕</button
								>
							</td>
						</tr>
